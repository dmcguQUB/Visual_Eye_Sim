...
export class DiagnosisQuestionsComponent extends AbstractTestComponent implements OnInit {
  ...
  constructor(
    ...
    this.questionType = QuestionType.Diagnosis;
  }
  

  override ngOnInit() {
    console.log("Diagnosis QuestionsComponent initialized.");
  ...
  sendUserScore(): void {
    // Example
    console.log("Send user score from DiagnosisQuestionsComponent");
 // Remove any default answers from the eyeTestAnswers array
 const cleanedDiagnosisAnswers = this.testAnswers.filter(answer => answer.questionId !== '');
 ...
 const testData: Test = {
     ...
     diagnosisTest: {
         score: this.points,
         answers: cleanedDiagnosisAnswers
     }
 };
