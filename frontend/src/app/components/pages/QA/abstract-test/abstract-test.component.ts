...
import { Test, EyeAndDiagnosisAnswer, InvestigationsAnswer } from 'src/app/shared/models/test';
  ...
  getQuestionsForUseCase() {
    ...
    if (this.useCaseId) {
      this.questionsService.getQuestionsByCaseStudyId(this.useCaseId).subscribe((res) => {
        ...
        this.questionList = res.filter(question => question.questionType === this.questionType);
        ...
        switch (this.questionType) {
          ...
          case QuestionType.Diagnosis:
            ...
            this.questionList.forEach(() => this.testAnswers.push({
              questionId: '', 
              answer: '', 
              correct: false,
            }));
